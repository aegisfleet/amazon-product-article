...
3. **価格・スペック・二重価格の事実確認**:
   - 過去参考価格からの不当な割引率（見せ金）表示があるか、カタログスペックに虚偽がないかを確認する。
...
| パターン | 実態 | 推奨対応 |
|---|---|---|
| **A. 同一商品のカラー・サイズ違い（正当な評価ブレ）** | 実売価格や仕様が同一だが加点/減点基準が不揃い | ファクトチェックを行い、**客観的事実に即した適正スコア・採点根拠に統一**する。 |
| **B. 実際の二重価格・虚偽スペック・サクラ警告** | 外部検証で実際にサクラ警告や悪質な見せ金が確認された | 減点（-15〜-20点）を適用した**適正な低スコア側に統一**（過大評価側を是正）。 |
...
### 5.3 不要・不当データの削除（必要な場合）
不当なバリエーションデータを削除する場合：
