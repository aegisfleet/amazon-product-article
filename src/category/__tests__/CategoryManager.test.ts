...
describe('CategoryManager', () => {
    ...
    test('childrenが文字列の配列でない場合にTypeErrorを投げる', () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(TypeError);
            expect((e as Error).message).toContain('Missing required field: children (string array) in category InvalidChildren');
        }
    });
    ...
    test('YAMLとJSONの両方に正しく出力する', () => {
        const categoryData = {
            categoryGroups: [
                { name: 'TestCat', slug: 'test-cat', children: ['ChildA'] }
            ]
        };
