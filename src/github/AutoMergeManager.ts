...
export class AutoMergeManager {
  ...
  validatePullRequest(pr: PullRequest): MergeDecision {
    ...
    validationResults.push({
      check: 'file-paths',
      passed: pathCheck,
      message: pathCheck ? 'All changed files are in allowed paths' : 'Some changed files are in restricted paths',
    });
  ...
  verifyJulesAuthor(pr: PullRequest): boolean {
    const isJules =
      pr.author === this.conditions.requiredAuthor ||
  ...
  verifyRequiredChecks(checkStatuses: Record<string, 'success' | 'failure' | 'pending'>): ValidationResult {
    const allPassed = this.conditions.requiredChecks.every((check) => checkStatuses[check] === 'success');
    ...
    return {
      ...
      passed: allPassed,
      message: allPassed ? 'All required checks passed' : `Missing or failed checks: ${missingChecks.join(', ')}`,
    };
  }
