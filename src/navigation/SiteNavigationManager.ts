...
export class SiteNavigationManager {
  ...
  generateCategoryIndex(articles: ArticleMetadata[]): CategoryIndex {
    ...
    // フィーチャー記事の選定
    const featuredArticles = articles.filter((a) => a.featured).slice(0, 10);
...
${urls
  ...
  applyFilters(params: FilterParams): FilterResult {
    ...
    // カテゴリフィルタ
    if (params.category) {
      filtered = filtered.filter((a) => this.slugify(a.category) === params.category);
    }

    // サブカテゴリフィルタ
    if (params.subcategory) {
      filtered = filtered.filter((a) => a.subcategory && this.slugify(a.subcategory) === params.subcategory);
    }

    // メーカーフィルタ
    if (params.manufacturer) {
      filtered = filtered.filter(
        (a) => a.manufacturer && a.manufacturer.toLowerCase() === params.manufacturer?.toLowerCase(),
      );
    }
    ...
    if (params.minRating !== undefined) {
      filtered = filtered.filter((a) => a.rating !== undefined && a.rating >= params.minRating!);
    }
  ...
  private createRatingFilters(): RatingFilter[] {
    return [5, 4, 3, 2, 1].map((stars) => ({
      stars,
      count: this.articles.filter((a) => a.rating !== undefined && Math.floor(a.rating) === stars).length,
    }));
  }
