...
describe('ArticleQualityManager', () => {
  ...
  describe('Property 11: Content Quality Validation', () => {
    // 有効な記事の生成器
    const validArticleArbitrary = fc
      .record({
...
*本記事にはアフィリエイトリンクが含まれています。*`;
    ...
    it('should always report error for articles without frontmatter', () => {
      const articleWithoutFrontmatter = fc.string({ minLength: 100 }).map((content) => `# タイトル\n\n${content}`);
      ...
      fc.assert(
        fc.property(articleWithoutFrontmatter, (article) => {
          ...
          const hasFrontmatterError = result.errors.some((e) => e.message.includes('フロントマター'));
