...
export class ArticleQualityManager {
   ...
   * セクション構造を検証
   */
  private validateSections(article: string): {
    errors: QualityIssue[];
   ...
   * コンテンツを検証
   */
  private validateContent(article: string): {
    errors: QualityIssue[];
    warnings: QualityIssue[];
    suggestions: QualityIssue[];
  } {
    ...
    // リンクチェック
    const affiliateLinkPattern = /\[[^\]]+\]\(https?:\/\/[^)\s]*?amazon[^)\s]*?\)/;
  ...
  private calculateCompleteness(
    ...
    const foundSections = sections.filter((s) => requiredSections.some((req) => s.title.includes(req))).length;
   ...
   * セクションの順序をチェック
   */
  private checkSectionOrder(sections: Array<{ title: string; content: string }>): {
    isCorrect: boolean;
