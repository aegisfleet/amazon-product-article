...
## 【最重要】信頼性とソースのガイドライン
- **URL検証**: 採用する全URLに対し、必ず \`curl\`、\`google_search\`、または \`view_text_website\` ツールを用いて対象のWebページを取得し、タイトルや本文に「確実に対象商品の名前が含まれているか」を直接チェックすること。404エラーだけでなく、無関係な商品ページ（ハルシネーション）のURLを採用することは絶対に許されません。
