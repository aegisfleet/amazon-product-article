...
4. **【型番・モデル名の一致確認（必須）】**: 取得したデータが本当に調査対象の商品のものかを必ず以下の手順で確認する。
...
5. **成果物の作成と徹底検証**: JSONファイル作成後、直ちに以下の検証を行う。
...
6. **外部調査と継続性**: Amazon 403エラー等でもGoogle検索等で調査を継続し、絶対に「調査不能」で終わらせない。情報が不足している場合は、自律的に検索キーワードを工夫して必要なエビデンスを揃える。
7. **推測ではなく根拠**: 商品仕様からの論理的推論は許容するが、架空のエピソード創作（ハルシネーション）は厳禁である。
8. **網羅的・完全構造化されたスペックの記載 (technicalSpecs)**:
...
9. **サイズ表記の統一**: サイズはメートル法（m, cm, mm等）のみを使用し、ディスプレイ画面サイズ以外のインチ表記は禁止。元データがインチのみの場合はメートル法に変換して記載する。
