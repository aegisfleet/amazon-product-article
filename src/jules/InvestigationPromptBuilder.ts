...
2. **評価カテゴリの選定**:
   ...
   **パターンA：一般商品 (性能・コスパ重視)**
   ...
   - コストパフォーマンス (-15 ～ +15点): 価格対性能、競合との価格差、容量あたりの単価（最重視）。※Amazon内の割引率は評価基準から外し、他の競合ECサイト（楽天市場、Yahoo!ショッピング等）の価格と比較してお得かどうかを評価すること。※異常割引率（例: 80%～90%）の商品は二重価格等の可能性が高いため評価対象から除外する
   ...
   **パターンB：安全性・信頼性重視商品 (美容・健康・食品・ベビー等)**
   ...
   - コストパフォーマンス (-10 ～ +10点): 安全性が確保された上での価格対効果。容量・サイズ違いによるコスパの変動もここで評価する。※Amazon内の割引率は評価基準から外し、他の競合ECサイトの価格と比較してお得かどうかを評価すること。※異常割引率（例: 80%～90%）の商品は二重価格等の可能性が高いため評価対象から除外する
...
- **ユーザーストーリー**: 実体験に基づく具体的エピソード（3行以上）。
  ...
  - **削除ルール**:
    ...
    - 根拠のないストーリーは不要である。
- **競合比較分析**: 主要競合6～8点のASIN特定と比較。※異常割引率（例: 80%～90%）の商品は二重価格等の可能性が高いため比較対象から除外する。調査を尽くしても6つに満たない場合は、ユーザーに確認を取らずそのまま作業を進めて良い。以下のフォーマットを厳守する：
  ---
  比較ポイント:
  共通点：[共通する特徴や仕様]
  相違点：[明確に異なる性能・価格・品質]
