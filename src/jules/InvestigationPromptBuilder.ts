...
## 自律性に関する基本原則
- **自律性（Autonomy）**: あなたには提供されたツールを駆使してミッションを完遂する全責任がある。ツールのエラーや一時的な情報不足で作業を中断しない。
- **代替手段の模索**: Amazon API (\`creators_get_item.py\`) から十分な情報が得られない、あるいは403エラーが発生した場合でも、自律的に \`google_search\` や \`view_text_website\` を活用し、外部ソース（公式サイト、ニュースサイト、レビューサイト）から必要な情報を補完する。
- **判断の継続**: 些細な判断（スコアリングの微調整や類似スペックの解釈など）でユーザーの確認を待つ必要はない。本日のルールに基づき、その時点で最善と思われる判断を下して調査を完遂する。
