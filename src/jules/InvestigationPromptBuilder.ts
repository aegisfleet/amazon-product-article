...
2. **既存データの徹底検証**: \`cat data/investigations/${this.product.asin}.json\` で既存データがあれば、その内容が現在でも妥当であるかを必ず検証し、**記載されている全URLが有効かつ関連性があるか、対象商品や競合商品の価格・スペックなどの事実情報が現在でも正確であるか（特に競合製品との価格比較）を再チェックし、古い場合は最新情報に更新**すること。
...
## 【最重要】信頼性とソースのガイドライン
- **URL検証**: 採用する全URLに対し、必ず \`curl\`、\`google_search\`、または \`view_text_website\` ツールを用いて対象のWebページを取得し、タイトルや本文に「確実に対象商品の名前が含まれているか」を直接チェックすること。
  - **必須チェック手順**: \`curl\` で取得したHTML全体を対象に \`grep -i "対象商品名"\` 等を行い、本当にその商品について言及されているか確認してください。\`head -n 20\` 等で冒頭だけを見るのは不十分です。
  - 404エラーだけでなく、無関係な商品ページ（例: 別の製品の価格.comページ等、ハルシネーション）のURLを採用することは絶対に許されません。
...
## 調査・分析項目
- **ユーザーストーリー**: 実体験に基づく具体的エピソード（3行以上）。実際のデータ（レビューや記事）が存在しない場合は、推測やプレースホルダーを記載するのではなく、そのユーザーストーリー自体を完全に削除（除外）してください。
    ...
    "userStories": [
      {
        "userType": "属性",
        "scenario": "状況",
        "experience": "3行以上の実体験（データが存在しない場合はストーリー自体を削除すること）",
