...
- **範囲表記の統一**: 数値の範囲（「10〜20g」など）を記述する際は、半角のチルダ \`~\` ではなく、全角の波ダッシュ \`～\` またはハイフン \`-\` を使用すること。
...
- **価格.comの調査ルール**:
  - 価格.comのURLや商品ID（\`K...\` や \`J...\` など）を推測（ハルシネーション）することは厳禁。
  - 必ず \`google_search\`（例: \`site:kakaku.com <商品名>\`）を用いて正確なURLを取得すること。
    ...
    "competitiveAnalysis": [
      {
        "name": "競合名",
        "asin": "ASIN(必須)",
        "priceComparison": "対象商品との価格差からの分析（価格は変動するので記載は不要。対象商品より高い場合はその価値があるか、安い場合は品質・性能に懸念がないかを記述）",
        "featureComparison": [
          "共通点：[テキスト可変]",
          "相違点：[テキスト可変]"
        ],
