...
2. **評価カテゴリの選定**:
   ...
   **パターンA：一般商品 (性能・コスパ重視)**
   ...
   - コストパフォーマンス (-15 ～ +15点): 価格対性能、競合との価格差、容量あたりの単価（最重視）。※Amazon内の割引率は評価基準から外し、他の競合ECサイト（楽天市場、Yahoo!ショッピング等）の価格と比較してお得かどうかを評価すること
   ...
   **パターンB：安全性・信頼性重視商品 (美容・健康・食品・ベビー等)**
   ...
   - コストパフォーマンス (-10 ～ +10点): 安全性が確保された上での価格対効果。容量・サイズ違いによるコスパの変動もここで評価する。※Amazon内の割引率は評価基準から外し、他の競合ECサイトの価格と比較してお得かどうかを評価すること
