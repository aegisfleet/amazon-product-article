...
2. **評価カテゴリの選定**:
   ...
   **価格の正確な認識と二重価格（見せ金）の排除（最重要）**:
   ...
   - 見せ金（ダミー商品）の判定: 同等機能の競合商品や、同じ製品の別カラー・別バリエーションと比較して、**機能的な差がないのに価格が不当に（数倍など）高い場合は、他のバリエーションを安く見せるための「見せ金（二重価格）」の疑いが強いとして大幅に減点（-15〜-20点）する**。

   **パターンA：一般商品 (性能・コスパ重視)**
   - 性能・機能 (-10 ～ +10点): スペック、実用性、使い勝手
   - コストパフォーマンス (-15 ～ +15点): 価格対性能、競合との価格差、容量あたりの単価（最重視）。※Amazon内の割引率は評価基準から外し、他の競合ECサイト（楽天市場、Yahoo!ショッピング等）の価格と比較してお得かどうかを評価すること。※異常割引率（例: 80%～90%）の商品は二重価格等の可能性が高いため評価対象から除外する。また、上記「見せ金」に該当する場合はここで大幅減点する。
   ...
   **パターンB：安全性・信頼性重視商品 (美容・健康・食品・ベビー等)**
   ...
   - コストパフォーマンス (-10 ～ +10点): 安全性が確保された上での価格対効果。容量・サイズ違いによるコスパの変動もここで評価する。※Amazon内の割引率は評価基準から外し、他の競合ECサイトの価格と比較してお得かどうかを評価すること。※異常割引率（例: 80%～90%）の商品は二重価格等の可能性が高いため評価対象から除外する。また、上記「見せ金」に該当する場合はここで大幅減点する。
...
2. **【型番・モデル名の一致確認（必須）】**: 取得したデータが本当に調査対象の商品のものかを必ず以下の手順で確認する。
...
3. **成果物の作成と徹底検証**: JSONファイル作成後、直ちに以下の検証を行う。
