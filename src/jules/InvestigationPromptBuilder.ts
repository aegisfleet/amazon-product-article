...
export class InvestigationPromptBuilder {
  ...
  private getParentAsinInfo(): string {
    if (!this.product.parentAsin) return '';
    return `- 親ASIN: ${this.product.parentAsin} (同一親ASINのバリエーション商品は、商品自体の本質的価値に関するスコアを加減点含めて揃えること)`;
  }
...
2. **評価カテゴリの選定**:
   ...
   **バリエーション商品（容量・サイズ違い）の評価の一貫性について（極めて重要）**:
   親ASINが同じなど、同一商品の容量・サイズ違い（バリエーション）を評価する場合は、商品そのものの本質的な価値（成分、味、匂い、基本的な使い勝手、ブランドの信頼性など）に対する評価は、どのバリエーションでも同一にすること。
   容量が違うだけで「匂いがきつい」等の本質的な特徴に対するペナルティや、「成分が豊富」等のボーナスが一方にだけ適用されたり、適用されなかったりするブレを絶対に防ぐこと。容量やサイズに関する評価（コストパフォーマンスや取り回しの良さなど）と、本質的な特徴に関する評価は明確に切り離して加減点を行うこと。
