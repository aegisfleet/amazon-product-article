...
2. **評価カテゴリの選定**:
   ...
   **価格の正確な認識と二重価格（見せ金・異常割引率）の厳格な排除（最重要）**:
   ...
   - **二重価格（見せ金・異常割引率）の確実な見破り方（Amazon内データで完結する判定手順）**:
     ...
     5. **【Amazon専売・ノーブランド品および物理的矛盾・誇大スペックの排除（極めて重要）】**:
        - **物理的矛盾・誇大スペックの判定**: 例えば「130g〜230gの軽量ボディに6,800mAh〜10,000mAhの超大容量バッテリー」「-25℃〜-30℃瞬間冷却」「120〜199段階風量」「17,000rpm超高速」「AI半導体冷却」など、物理的・工学的に矛盾する、あるいは客観的根拠のないマーケティング的誇大表記を絶対に鵜呑みにして加点してはならない。これらはスペック誇大表示・信頼性欠如として**性能・信頼性の観点から大幅減点（-10〜-15点）**する。
        - **ノーブランド品・新興ブランド品のスコア上限**: 企業の実態や国内サポート体制が不透明な新興ECブランド・ノーブランド品は、カタログスペックの加点を最大でも+5〜+10点程度に抑え、信頼性・サポート不透明さ（-5点）やスペック過大表記リスク（-10点）を必ず適用し、**基本的に60〜65点以下（60点台前半）をスコア上限**とすること。決して80点以上の高評価をつけてはならない。
