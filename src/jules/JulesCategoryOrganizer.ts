...
5. **カテゴリのソート順（重要）**
   - 各親カテゴリ内の \`categories\` 配列は **文字コード順（Unicodeコードポイント順）** で並べてください
   - JavaScriptでは以下のようにソートします:
     \`\`\`javascript
     categories.sort()
     \`\`\`
   - **ソート順の特徴**: 英数字（0-9, A-Z, a-z）→ ひらがな → カタカナ → 漢字 の順で並びます
   - **具体例（「家電」カテゴリの正しいソート順）**:
     \`\`\`json
     [
       "HEPA空気清浄機",
     ...
     \`\`\`
   - 既存の \`categorygroups.json\` のソート順を参考にしてください
