...
4. **判断基準**
   ...
   - もし適切なカテゴリの判断に迷った場合は、\`data/cache/paapi-product-cache.json\` を参照して商品情報を確認してください（ASINや商品タイトルが含まれています）。

    - **自動ソートの実装**:
      手動でソートを行う必要はありません。カテゴリーの追加・移動を行った後、必ず以下のコマンドを実行してファイルを自動整理してください：
      \`\`\`bash
      npm run sort:categories
      \`\`\`

    - **ソートのルール（内部仕様）**:
      英数字 (0-9, A-Z) -> ひらがな -> カタカナ -> 漢字 の順（Unicodeコードポイント順）に並びます。
...
### 1. ファイル編集
\`data/categorygroups.json\` に未登録カテゴリーを振り分けてください。

### 2. 自動ソートの実行（必須）
編集後、必ず以下のコマンドを実行してソート順を正しく整えてください：
\`\`\`bash
npm run sort:categories
\`\`\`
...
git diff --staged
\`\`\`
※ 変更が正しくステージングされ、カテゴリーが期待通りの親カテゴリーに移動していることを確認してください。
...
> - 変更が許可されている実ファイルは \`data/categorygroups.json\` のみです
> - ただし、手順として \`npm run sort:categories\` の実行は許可（推奨）されています
...
- ビルド（\`npm run build\`）やテスト（\`npm test\`）の実行は不要です
- \`npm run sort:categories\` による自動ソートのみ実行してください
