...
5.    - **カテゴリのソート順（最重要）**
    - 各親カテゴリ内の \`categories\` 配列は **Unicodeコードポイント順（JavaScriptのデフォルトのsort順）** で並べてください。
    - **ソートの特性**:
      - **読み（あいうえお順）は考慮しないでください。**
      - 英数字（0-9, A-Z） -> ひらがな -> カタカナ -> 漢字 の順に明確に分かれます。
      - カタカナとひらがなが混ざることはありません。
    - **検証方法**:
      自信がない場合は、必ず以下のコマンドをターミナルで実行して結果を確認してください：
      \`\`\`bash
      node -e "console.log(JSON.stringify(['カテゴリ1', 'カテゴリ2', ...].sort(), null, 2))"
      \`\`\`
    - **具体例（「美容・健康」カテゴリの正しいソート順）**:
      \`\`\`json
      [
        "おしゃれカラコン",
        "わき式体温計",
      ...
      \`\`\`
      ※ ひらがな（あ...）、カタカナ（ア...）、漢字の順に並びます。Unicodeコードポイント順（デフォルトのsort順）です。
    - **注意**: \`localeCompare\` などは絶対に使用せず、デフォルトの \`sort()\` を使用してください。
