...
5. **カテゴリのソート順（重要）**
   - 各親カテゴリ内の \`categories\` 配列は **日本語ロケールのアルファベット順** で並べてください
   - JavaScriptでは以下のコードでソートします:
     \`\`\`javascript
   ...
   - **具体例（「美容・健康」カテゴリの正しいソート順）**:
     ...
     \`\`\`
     ※ 「アイクリーム」（カタカナ）が「おしゃれカラコン」（ひらがな）より前に来ます
   - **注意**: デフォルトの \`sort()\` ではなく、必ず \`localeCompare('ja')\` を使用してください
