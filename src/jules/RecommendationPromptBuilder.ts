...
export class RecommendationPromptBuilder {
  ...
  private loadSaleCandidatesPromptSection(): string {
    ...
    try {
      ...
      const candidateLines = data.candidates.slice(0, 35).map((item, index) => {
...
以下の商品は、過去のキャッシュから抽出された、適正な割引率（二重価格の疑いがない健全なセール）と信頼性を持つおすすめ候補である。
...
3. **${n}商品の厳選と詳細調査**:
   - 検索結果および事前抽出候補から、明確なフック（大幅値引き、新発売、トレンド合致、高評価定番など）がある商品を、カテゴリが被らないように${n}点ピックアップせよ。
   ...
   - **異常割引率の除外**: 異常割引率（例: 70%～90%）や常時高割引率のノーブランド品は二重価格の可能性が高いため評価・選定対象から除外すること。
   ...
   - **セール品と高スコア商品の組み合わせ（10選の構成）**:
     - セール品だけで10個揃わない場合や、セール対象に魅力的な商品が不足している場合は、リポジトリ内の高スコア（Front Matterの \`score\` が75点以上）の定番人気商品や、トレンドの新製品を組み合わせることで、必ず${n}個の厳選リストを完成させよ。
