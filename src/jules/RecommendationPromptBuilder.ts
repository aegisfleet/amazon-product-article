...
最も重要なルールは、**「なぜ今日おすすめするのか（Why Buy Now）」の根拠となる『情報元（エビデンス）』を必ず明記すること**だ。客観的な事実に基づかない推奨は無価値である。
...
1. **トレンド・セール情報の収集と「エビデンス」の確保**:
   ...
   - その際、「どのサイト（URL）で話題になっていたか」「どの公式ページでセールが告知されているか」という**情報元（ソース）のリンクや名称を必ず記録**しておくこと。
2. **多角的な商品検索（カテゴリの分散）**:
   - 意図的にジャンルを分散させ（例：PC周辺機器、食品、日用品、家電、エンタメなど）、それぞれ異なるキーワードで \`python scripts/creators_search_items.py\` を実行せよ。1つのテーマに偏らないように注意すること。
3. **10商品の厳選と詳細調査**:
   - 検索結果から、明確なフック（大幅値引き、新発売、トレンド合致など）がある商品を、カテゴリが被らないように10点ピックアップせよ。
   - **注意**: 割引率が 80% や 90% など異常に高い商品は、元の価格を不正に釣り上げている可能性があるため、安易に選定しないこと。信頼できる値引きであると判断できる場合のみ選定の候補とせよ。
   - \`python scripts/creators_get_item.py ASIN\` を実行し、詳細なスペックや現在の価格情報を裏付けること。
4. **推奨理由と情報元の言語化**:
