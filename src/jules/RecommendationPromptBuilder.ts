...
4. **推奨理由と情報元の言語化および品質スコアの確認**:
   ...
   - サイト内レビューで60点未満の低スコア商品であっても、割引率が極めて高い（半額以上など）など「今買うべき理由」が特に強い場合は掲載してよい。その場合、\`scoreDisclaimer\` フィールドに「価格重視なら候補」「品質スコアは低め」といった、ユーザーが誤解しないための注意喚起・アドバイスを設定すること。スコアが十分高い場合や、レビュー記事が存在しない場合は \`scoreDisclaimer\` を null または空文字にせよ。
   - 選定基準におけるこの商品の順位・掲載理由（例: \`割引率重視\`, \`高スコア\`, \`日用品の実用性\`, \`低スコアだが半額以上のため掲載\` など）を簡潔に分析し、\`rankReason\` に設定せよ。
  ...
  "recommendations": [
    {
      ...
      "rankReason": "選定基準におけるこの商品の掲載・順位理由（例: 割引率重視、高スコア、実用性重視、低スコアだが半額以上の価格メリットなど。20文字以内で簡潔に）",
      "scoreDisclaimer": "低スコア商品（目安としてサイト内レビューで60点未満）に対する注意ラベル。例: 「価格重視なら候補」「品質スコアは低め」「実用性重視」など。問題ない、あるいはレビュー記事がない場合は null または空文字",
      "source": {
