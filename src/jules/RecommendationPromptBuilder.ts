...
1. **トレンド・セール情報の収集と「エビデンス」の確保**:
   ...
   - その際、「どのサイト（URL）で話題になっていたか」「どの公式ページでセールが告知されているか」という**情報元（ソース）のリンクや名称を必ず記録しておく**。
2. **多角的な商品検索（カテゴリの分散）**:
   - 意図的にジャンルを分散させ（例：PC周辺機器、食品、日用品、家電、エンタメなど）、それぞれ異なるキーワードで \`python scripts/creators_search_items.py\` を実行せよ。1つのテーマに偏らないように注意する。
3. **10商品の厳選と詳細調査**:
   ...
   - \`python scripts/creators_get_item.py ASIN\` を実行し、詳細なスペックや現在の価格情報を裏付ける。
4. **推奨理由と情報元の言語化**:
...
- **検証**: 成果物を生成した後、必ず \`python scripts/validate_artifact.py data/recommendations/today.json\` を実行し、構造の正当性やリンク切れ（画像含む）がないか確認せよ。
- ハルシネーション（存在しない商品、架空のセール、実在しないニュースやURLの創作）は厳禁だ。必ず実在するエビデンスへのリンクを記載せよ。
...
それでは、幅広いカテゴリを対象に情報収集を開始し、明確なエビデンスを持った魅力的な10個の商品を見つけ出せ。`;
  }
}
