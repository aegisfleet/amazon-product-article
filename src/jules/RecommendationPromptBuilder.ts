...
4. **選定理由の明記**:
   - 選定基準におけるこの商品の順位・掲載理由（例: \`特選タイムセール中\`, \`スマイルSale対象\`, \`季節需要・限定割引\`, \`タイムセールでお買い得\` など）を簡潔に分析し、\`rankReason\` に設定せよ。
  ...
  "recommendations": [
    {
      ...
      "whyBuyNow": "なぜ『今日（今）』買うべきなのか（例：特選タイムセールでお買い得、スマイルSale開催中、季節の備えとして今が最適など。※「いつ買っても良い」等の理由は不可）",
      "rankReason": "選定基準におけるこの商品の掲載・順位理由（例: 特選タイムセール中、スマイルSaleでお買い得、季節需要・限定割引など。20文字以内で簡潔に）",
      "scoreDisclaimer": null,
      "source": {
        "name": "情報元の名称（例：Amazon スマイル Sale、Amazon タイムセール等）",
