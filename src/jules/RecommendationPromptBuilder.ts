...
3. **順位付け（1〜${n}位）と本日買う理由（Why Buy Now）の厳格化**:
   - **「記事スコアの高い順」に並べることは厳禁**。1位から${n}位の並び順は、「**今この瞬間に買うメリットの大きさ（割引率の高さ・限定セール・季節の緊急性やタイムリー需要）**」の順にランキングせよ。
   - 【特選セール・限定セール枠（5〜7品）】：高スコアかつ本日リアルタイムで特選タイムセールやプライム限定セール中の商品
   - 【実用・季節トレンド・タイムリー枠（3〜5品）】：タイムセールや割引対象、または季節イベント需要で「今このタイミングで買う明確なメリット」があるお買い得商品
   - **いつでも買える単なる定番品の選定禁止 (重要)**: セールや割引、タイムリーな需要がない「いつでも買える定番品」を本日の注目商品として選定してはならない。必ず「本日お買い得である客観的根拠（セールバッジ・割引率等）」または「今買うべき季節性・トレンド」が存在する商品を厳選せよ。
  ...
  "recommendations": [
    {
      ...
      "whyBuyNow": "なぜ『今日（今）』買うべきなのか（例：現在開催中の特選タイムセールで過去最安水準、9月1日の防災の日に向けた備蓄見直しとして最適など。※単にスコアが高いから等の理由は不可）",
      "rankReason": "選定基準におけるこの商品の掲載・順位理由（例: 35% OFF 特選セール、防災の日 備蓄見直し、プライム限定割引など。20文字以内で具体的に）",
      "scoreDisclaimer": null,
      "source": {
        "name": "情報元の名称（例：Amazon 特選タイムセール、Amazon プライム限定セール、季節トレンド等）",
