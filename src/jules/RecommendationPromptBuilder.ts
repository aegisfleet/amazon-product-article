...
4. **順位付け（1〜${n}位）と本日買う理由（Why Buy Now）の厳格化**:
   ...
   - **実在するASINと最新API状態の絶対保証**: リストに追加する商品は、事前抽出リストの候補を最優先で採用し、可能であれば \`uv run python scripts/creators_get_item.py ASIN\` を実行して詳細なスペック、**現在の最新価格、および現在の最新セールバッジ (\`dealBadge\`)** を取得せよ。APIから取得していない架空のASINは絶対に生成・追加してはならない。
   - **API実行不可時の自律フォールバック**: もし環境変数や認証エラーで \`creators_get_item.py\` が実行できない環境である場合でも、作業を中断せず事前抽出リストに記載された ASIN、商品名、カテゴリ、実売価格、セール情報を活用して自律的にリストを構成せよ。
5. **選定理由の明記と多様化**:
  ...
  "recommendations": [
    {
      ...
      "imageUrl": "画像のURL（APIから取得できた場合のみ。不明またはAPI未実行時は必ず null とせよ。複数の商品に同一の画像URLをコピペすることは厳禁。null の場合はサイト側でキャッシュから自動補完される）"
    }
  ],
  "searchContext": {
...
※ **フィールド値に関する重要ルール**:
- **price**: 必ず 0 より大きい実際の販売価格（日本円の数値）を設定すること（\`price: 0\` は禁止）。APIから取得できない場合でも、上記の候補リストに記載された価格を設定せよ。
- **imageUrl**: 同一の画像URLを複数商品にコピペすることは厳禁とする。APIから正式に取得できない場合や不明な場合は、無理にURLを設定せず必ず \`null\` とせよ。
