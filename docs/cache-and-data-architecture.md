...
### 4.1 複数PRの同時多発マージに対する耐性と自動マージ機構

複数のPRが短時間の間に連続・同時多発的にマージされた場合でも、デプロイジョブの失敗やキャッシュの欠落を防ぐ二重のフェイルセーフが備わっている。

1. **Concurrency制御による最新ビルド優先 (`deploy-articles.yml`)**:
   - `concurrency: group: "deploy-articles-${{ github.ref }}", cancel-in-progress: true` により、連続マージ時に先行する古いビルドジョブは自動キャンセルされ、最新コミットのビルドが優先実行される。
2. **プッシュ時の競合検知と自動リトライ (`git-commit-push.sh`)**:
   - キャッシュのプッシュ時にリモート（`main` ブランチ）が進んでいた場合、最大3回のリトライループを実行する。
   - 単純な再試行だけでなく、リモートの最新状態を `git fetch` し、現在のキャッシュ変更と `git merge` を試みる。
3. **スマートキャッシュ自動マージ (`merge-product-cache.ts`)**:
   - 複数PRが同時にキャッシュを更新したことで `data/cache/paapi-product-cache.json` にGitコンフリクト（競合）が発生した場合、スクリプトが自動介入する。
   - Gitインデックスから自ブランチ（`ours`）とリモートブランチ（`theirs`）の両方のキャッシュJSONを取り出し、`src/scripts/maintenance/merge-product-cache.ts` を実行する。
   - **ASIN単位のタイムスタンプ比較マージ**:
     - 片方にのみ存在するASIN: そのまま新規追加。
     - 双方に存在する同一ASIN: 各エントリの `timestamp` を比較し、**より新しいタイムスタンプのデータ**を自動採用。
   - これにより、一方のビルドで取得した最新価格や画像データが他方の変更で上書き消失することなく、両者の成果が漏れなく統合された状態で自動コミット・プッシュが完了する。
