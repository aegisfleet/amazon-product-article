...
### 1.1 概要
訪問者がサイト上の「📝 調査リクエスト」ボタンからGoogleフォームを開き、記事化を希望するAmazon商品のURLを送信すると、システムが定期的（毎日13:00 / 21:00 JST）にリクエストを自動収集し、商品調査および比較記事の作成・公開までを自律的に実行する。
...
sequenceDiagram
    ...
    Note over GHA: 毎日 13:00 / 21:00 JST に自動起動
...
| **GHA ワークフロー** | `.github/workflows/investigate-user-requests.yml` | 1日2回の定期実行パイプライン。調査からJulesセッション起動までを一括オーケストレーション。 |
...
- **自動復旧**: 24時間以上経過すると、次回の定期実行（13:00 / 21:00）で自動的にタイムアウト判定され、再調査対象としてリトライされる。
